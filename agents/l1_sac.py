...
class L1SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.l1_lambda = 1e-4  # L1 penalty weight
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        # L1 Penalty on first layer state features only
        l1_reg = torch.norm(self.critic.linear1.weight[:, :num_inputs], p=1) + \
                 torch.norm(self.critic.linear4.weight[:, :num_inputs], p=1)
        
        qf_loss = qf_loss + self.l1_lambda * l1_reg
        ...
        # L1 Penalty on policy network input
        l1_reg_policy = torch.norm(self.policy.linear1.weight, p=1)
        policy_loss = policy_loss + self.l1_lambda * l1_reg_policy
